...
const getUserNotes = (userId: number): Record<string, Record<string, any>[]> => ({
    notes: notesData.filter(({ userId: notesUserId }) => (
        notesUserId === userId
    )),
    sharedNotes: notesData.filter(({ userId: notesUserId, isShared }) => (
        notesUserId !== userId && isShared
    ))
});

const sharedNote = notesData.find(({ userId, isShared }) => (
    userId !== Api.userId && isShared
));
if (!sharedNote) {
    throw new Error('Could not find any shared notes');
}

const unsharedNote = notesData.find(({ userId, isShared }) => (
    userId !== Api.userId && !isShared
));
if (!unsharedNote) {
    throw new Error('Could not find any shared notes');
...
describe('[API] Notes', () => {
    ...
    describe('PUT /notes/:id/(up|down)', () => {
        ...
        it('Should move position of a note', async () => {
            ...
            for (const { action, position } of moveData) {
                ...
                const targetPosition = position === 'last' ? maxPosition : Number(position);
                ...
                const otherNotePosition = action === 'down' ? targetPosition + 1 : targetPosition - 1;
                const otherNote = notes.find(({ position: notePosition }) => (
                    notePosition === otherNotePosition
                ));
                if (!note || !otherNote) {
                    throw new Error('Could not get notes to test position move');
                }
                const expectedNotePosition = action === 'down' ? (
                    note.position + 1
                ) : (
                    note.position - 1
                );
                const expectedOtherNotePosition = action === 'down' ? (
    ...
    describe('DELETE /notes/:id', () => {
        ...
        it('Should delete a note', async () => {
            ...
            const { notes: updatedNotes } = await getApiNotes(notes[0].sessionId);
